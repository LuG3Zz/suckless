...
struct Client {
  ...
  int bw, oldbw;
  unsigned int tags;
  int isfixed, iscentered, isfloating, wasfloating, isurgent, neverfocus,
...
void movecenter(const Arg *arg) {
  ...
  if (selmon->lt[selmon->sellt]->arrange && !c->isfloating)
    togglefloating(NULL);
  nh = (selmon->wh / 2) - (c->bw * 1);
  nw = (selmon->ww / 2) - (c->bw * 1);
  nx = (arg->ui / 4) - 1;
  ny = (arg->ui / 4) - 1;
  if (nx < 0)
    nx = selmon->wx;
  ...
  // XWarpPointer(dpy, None, c->win, nx, ny, 0, 0, nw / 2, nh / 2);
}
